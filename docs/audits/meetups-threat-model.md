...
| MT-02 | A blocked organizer or participant uses a public event, invitation, RSVP, participant list, message, or exact location as a contact bypass. | Harassment, identity disclosure, and real-world co-location. | Query, Policy, invitation, admission, messaging, and reveal boundaries reuse `SocialAccountBlock`; an existing active participation is safely removed, access revoked, and waitlist promotion audited. |
| MT-03 | A browser submits a foreign, inactive, denied, pending, invited, revoked, future, or expired pet-manager relationship. | Private pet disclosure and false participation. | Inside the admission/review/promotion transaction require an active user, active PetProfile, and current `PetProfileAccess::allows(..., View)`. Ordinary caregiver participation does not require `ManageSocial`; acting or publishing as the pet actor still does. |
...
| MT-06 | Concurrent join, approval, leave, or promotion relies on status counts rather than capacity allocation. | Overbooking, two promotions for one place, or leaked capacity. | Typed human-capacity pool and allocation rows are locked in canonical order; two-process final-slot and promotion races prove allocated quantity never exceeds capacity. |
...
| MT-13 | Cancellation, removal, reschedule, or update writes state but notification/audit delivery races or leaks exact location. | False or duplicate notices, missing safety update, private data disclosure. | Transition and privacy-safe notification intent are stored in the same transaction; recipient-locale delivery runs after commit and deduplicates by transition version. |

...
**Architecture:** Two invokable controllers receive complete view-data arrays from `App\Services\PreviewService`. A page-layout-agnostic Blade shell wraps both the existing three-column feed and the new responsive profile, while focused anonymous components render profile identity, facts, gallery, and owner content.

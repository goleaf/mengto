...
| SYS-AUTH-005 | Every first-party product page, mutation, token share, Livewire upload/preview, and product-media response requires an active authenticated account; verified-email proof is additionally required when the configured email-verification mode is enabled. | Anonymous visitor, member; all portal resources | Anonymous HTML redirects and JSON returns 401 before model binding or persistence; inactive accounts always fail closed; enabled mode rejects unverified accounts, while disabled mode deliberately stamps active accounts without claiming independent email-ownership proof; account entry, static assets, and minimal health remain available. |
| SYS-AUTH-006 | A newly registered account with an incomplete onboarding row cannot access ordinary portal resources until forward-only, optimistic, idempotent account setup is complete. | Active conditionally verified member; onboarding and all portal resources | The onboarding boundary runs after portal access and before bindings, returns localized conflict responses for JSON/mutations, permits only the exact account-flow and canonical pet-create bridge, preserves a same-origin intended GET, and treats missing rows as legacy exemption. |

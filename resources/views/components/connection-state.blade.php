...
@if ($item['following'] || $item['favorite'] || $item['muted'] || $item['notification_level'])
    <div class="connection-state" aria-label="Subscription status">
        @if ($item['following'])
            <x-status-badge label="Following" icon="user-check" tone="mint" />
        @endif
        @if ($item['favorite'])
            <x-status-badge label="Favorite" icon="star" tone="sun" />
        @endif
        @if ($item['muted'])
            <x-status-badge label="Muted" icon="volume-x" tone="surface" />
        @endif

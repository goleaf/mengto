...
<article role="listitem" data-discover-result class="pc-panel pc-panel--clip grid sm:grid-cols-[11rem_minmax(0,1fr)]">
    ...
    <div class="min-w-0 p-4 sm:p-5">
        <div class="flex items-center justify-between gap-3">
            ...
            <p class="truncate text-xs font-semibold text-paw-muted">{{ $result['meta'] }}</p>
        </div>

        <h3 class="mt-2 break-words text-lg font-semibold text-paw-ink">
            <x-pet-social.text-link :href="route($result['route'])">
                {{ $result['title'] }}
            </x-pet-social.text-link>
        </h3>
        <p class="mt-2 text-sm leading-6 text-paw-muted">{{ $result['description'] }}</p>

        <x-pet-social.tag-list :items="$result['tags']" empty="Open to local connections." class="mt-3" />

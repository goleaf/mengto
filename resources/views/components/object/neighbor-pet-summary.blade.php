...
<section data-section="neighbor-pet" {{ $attributes->merge(['class' => 'panel panel--clip']) }}>
    <div class="grid sm:grid-cols-[14rem_minmax(0,1fr)]">
        ...
        <div class="p-5">
            ...
            <x-ui.tag-list :items="$pet['traits']" empty="Routine notes unavailable." class="mt-4" />

            <dl class="mt-5 grid gap-3 border-t border-paw-line pt-4 sm:grid-cols-3">
                @forelse ($pet['routine'] as $item)
                    <div>
                        <dt class="text-xs font-semibold text-paw-muted">{{ $item['label'] }}</dt>

...
<x-object.directory-card data-group-card class="group-card" {{ $attributes }}>
    <x-slot:media>
        <x-ui.card-media
            :src="$group['image']"
        ...
        >
            <div class="group-card__badges">
                <x-ui.status-badge :label="$group['category']" />
                ...
                @if ($group['official'] ?? false)
                    <x-ui.status-badge label="Official" icon="badge-check" tone="mint" />
                @endif
            </div>
        </x-ui.card-media>
    </x-slot:media>
    ...
    @if ($group['recommendation_reason'] ?? null)
        <x-object.recommendation-reason
    ...
    <x-ui.tag-list :items="$group['tags']" empty="Open to new neighbors." reserve class="group-card__tags" />

    <dl class="group-card__metrics">
        <div class="min-w-0">
            <dt>
                <x-ui.icon-text icon="users" class="text-xs">
                    Community
                </x-ui.icon-text>
            </dt>
        ...
        <div class="min-w-0">
            <dt>
                <x-ui.icon-text icon="activity" class="text-xs">
                    Activity
                </x-ui.icon-text>
            </dt>
    ...
    @if ($group['next_event'] ?? null)
        <x-ui.icon-text icon="calendar-clock" class="group-card__event">
            {{ $group['next_event'] }}
        </x-ui.icon-text>
    @endif
    ...
    <div class="group-card__footer">
        <div class="group-card__organizer">
            <x-ui.initials-avatar :initials="$group['organizer_initials']" />
            <div>

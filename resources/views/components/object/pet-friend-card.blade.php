...
<article id="friend-{{ $item['key'] }}" class="pet-friend-card">
    <header class="pet-friend-card__header">
        <x-object.connection-identity :item="$item" />
        <x-ui.status-badge
    ...
    @if ($item['relationship'])
        <div class="pet-friend-card__relationship">
            ...
            @if ($item['relationship']['intents'] !== [])
                <x-ui.tag-list :items="$item['relationship']['intents']" />
            @endif
        </div>
    ...
    <x-object.pet-compatibility :compatibility="$item['compatibility']" />

    @if ($item['request_form'])
        <x-feature.pet-friend-request-form :item="$item" :endpoint="$endpoint" />
    @endif

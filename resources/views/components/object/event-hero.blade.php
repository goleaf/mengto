...
<section class="event-hero" aria-labelledby="event-title">
    <div class="event-hero__media">
        ...
        <div class="event-hero__badges">
            <x-ui.status-badge :label="$event['status_label']" :tone="$event['status_tone']" />
            <x-ui.status-badge :label="\Illuminate\Support\Str::headline($event['format'])" tone="surface" />
            @if ($event['verification_label'])
                <x-ui.status-badge :label="$event['verification_label']" icon="badge-check" tone="safe" />
            @endif
        </div>
    </div>
    ...
    <div class="event-hero__body">
        ...
        <x-object.detail-meta-list :items="$event['meta']" />
        <x-ui.tag-list :items="$event['tags']" empty="No event topics." class="event-hero__tags" />
        <x-ui.stat-grid
            :items="$event['stats']"
            label="Event summary"

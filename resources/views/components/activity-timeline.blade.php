...
<section data-section="activity-timeline" {{ $attributes->class(['panel', 'panel--clip']) }}>
    <h2 class="sr-only">{{ __('ui.activity_updates') }}</h2>

    <form method="GET" action="{{ route('notifications.index') }}" class="border-b border-paw-line p-4 sm:p-5">
        <x-panel-heading title="{{ __('ui.activity_filters') }}" :meta="__('presentation.new_count', ['count' => $unreadCount])" />

        <x-filter-group
            :filters="$filters"
            :active="$activeFilter"
            label="{{ __('ui.activity_filters') }}"
            submit
            class="mt-4"
        />
    </form>
    ...
    @empty
        <x-empty-state
            icon="bell-off"
            title="{{ __('ui.all_caught_up') }}"
            description="{{ __('ui.new_brand_activity_will_appear_here') }}"
            class="m-4 sm:m-5"
        />
    @endforelse
</section>

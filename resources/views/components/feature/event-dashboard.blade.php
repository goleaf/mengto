...
<div class="event-dashboard">
    ...
    @if ($activeTab === 'tickets')
        <div class="event-dashboard__columns event-dashboard__columns--registration">
            <x-feature.event-registration-panel :event="$event" :registration="$registration" />

            <div class="event-dashboard__aside">
                <x-ui.content-panel section="event-checklist" eyebrow="Before confirming" title="Registration checklist">
    ...
    @elseif ($activeTab === 'schedule')
        <x-ui.content-panel section="event-schedule" eyebrow="Event program" title="Schedule">
    ...
    @elseif ($activeTab === 'attendees')
        <x-ui.content-panel section="event-attendees" eyebrow="Privacy-aware directory" title="Registered people">
            <div class="event-people section-body" role="list">
                @forelse ($content['attendees'] as $person)
                    <article class="event-person" role="listitem">
                        <x-ui.initials-avatar :initials="$person['initials']" :tone="$person['tone']" />
                        <div>
                            <h3>{{ $person['name'] }}</h3>
                            <p>{{ $person['detail'] }}</p>
                        </div>
                        <x-ui.status-badge :label="$person['badge']" tone="surface" />
                    </article>
                @empty
    ...
    @elseif ($activeTab === 'pets')
        <x-ui.content-panel section="event-pets" eyebrow="Owner-controlled profiles" title="Pets attending">
            <div class="event-people section-body" role="list">
                @forelse ($content['pets'] as $pet)
                    <article class="event-person" role="listitem">
                        <x-ui.initials-avatar :initials="$pet['initials']" :tone="$pet['tone']" />
                        <div>
                            <h3>{{ $pet['name'] }}</h3>
                            <p>{{ $pet['detail'] }}</p>
                        </div>
                        <x-ui.status-badge :label="$pet['badge']" tone="surface" />
                    </article>
                @empty
    ...
    @elseif ($activeTab === 'chat')
        <div class="event-dashboard__columns">
            <x-ui.content-panel section="event-chat" eyebrow="Registered participants" title="Event chat">
                <div class="event-chat section-body" role="log" aria-label="Event messages">
                    @forelse ($content['chat'] as $message)
                        <article class="event-message">
                            <x-ui.initials-avatar :initials="$message['initials']" :tone="$message['tone']" />
                            <div>
                                <header>
            ...
            <x-ui.content-panel section="event-message-composer" title="Send a message" meta="Owners speak for pet profiles">
                @if ($registration['registration'] || $event['managed_by_current_user'])
                    <form method="POST" action="{{ route('pet-social.actions.perform') }}" class="event-message-form section-body">
                        ...
                        <x-ui.action-control type="submit" label="Send message" icon="send" variant="primary" />
                    </form>
                @else
                    <x-ui.notice
                        icon="lock-keyhole"
    ...
    @elseif ($activeTab === 'announcements')
        <div class="event-dashboard__columns">
            <x-ui.content-panel section="event-announcements" eyebrow="Organizer updates" title="Announcements">
            ...
            @if ($event['managed_by_current_user'])
                <x-ui.content-panel section="event-announcement-composer" title="Publish an update">
                    <form method="POST" action="{{ $organizerTools['announcement_action'] }}" class="event-message-form section-body">
                        ...
                        <x-ui.action-control type="submit" label="Publish update" icon="megaphone" variant="primary" />
                    </form>
                </x-ui.content-panel>
            @endif
        </div>
    @elseif ($activeTab === 'location')
        <div class="event-dashboard__columns">
            <x-ui.content-panel section="event-location" eyebrow="Privacy-aware place" title="{{ $content['location']['general'] }}">
            ...
            <x-ui.content-panel section="event-accessibility" eyebrow="Arrival planning" title="Access and nearby help">
                <x-ui.definition-list :items="$content['location']['details']" strong class="section-body" />
            </x-ui.content-panel>
        </div>
    @elseif ($activeTab === 'media')
        <x-ui.content-panel section="event-media" eyebrow="Consent-aware album" title="Event photos">
            ...
            @if ($registration['registration'] || $event['managed_by_current_user'])
                <form method="POST" action="{{ route('pet-social.actions.perform') }}" class="event-photo-form section-body">
                    ...
                    <x-ui.action-control type="submit" label="Add sample photo" icon="image-plus" variant="paper" />
                </form>
            @endif
        </x-ui.content-panel>
    @elseif ($activeTab === 'rules')
        <div class="event-dashboard__columns">
            <x-ui.content-panel section="event-rules" eyebrow="Participation boundary" title="Rules">
            ...
            <x-ui.content-panel section="event-files" eyebrow="Current versions" title="Files and guides">
    ...
    @elseif ($activeTab === 'reviews')
        <div class="event-dashboard__columns">
            <x-ui.content-panel section="event-reviews" eyebrow="Verified attendance" title="Event feedback">
                <div class="event-reviews section-body">
                    @forelse ($content['reviews'] as $review)
                        <article>
                            <header>
                                <x-ui.initials-avatar :initials="$review['initials']" :tone="$review['tone']" />
                                <div>
                                    <h3>{{ $review['title'] }}</h3>
            ...
            <x-ui.content-panel section="event-review-form" title="Share private or public feedback">
                @if (($registration['status'] ?? null) === 'checked_in')
                    <form method="POST" action="{{ route('pet-social.actions.perform') }}" class="event-message-form section-body">
                        ...
                        <x-ui.action-control type="submit" label="Publish review" icon="star" variant="primary" />
                    </form>
                @else
                    <x-ui.notice
                        icon="badge-check"
    ...
    @elseif ($activeTab === 'manage' && $event['managed_by_current_user'])
        <div class="event-dashboard__manage">
            <x-ui.content-panel section="event-analytics" eyebrow="Aggregate only" title="Registration funnel">
                <x-ui.stat-grid
            ...
            <div class="event-dashboard__columns">
                <x-ui.content-panel section="event-applications" title="Applications" meta="{{ count($content['applications']) }} visible">
                    <div class="event-decisions section-body">
                        @forelse ($content['applications'] as $application)
                            <article>
                                <x-ui.initials-avatar :initials="$application['initials']" :tone="$application['tone']" />
                                <div>
                                    <h3>{{ $application['name'] }}</h3>
                                    <p>{{ $application['detail'] }}</p>
                                    <x-ui.status-badge :label="$application['status']" tone="surface" />
                                </div>
                ...
                <x-ui.content-panel section="event-waitlist" title="Waitlist">
                    <div class="event-decisions section-body">
                        @forelse ($content['waitlist'] as $candidate)
                            <article>
                                <x-ui.initials-avatar :initials="$candidate['initials']" :tone="$candidate['tone']" />
                                <div>
                                    <h3>{{ $candidate['name'] }}</h3>
                                    <p>{{ $candidate['detail'] }}</p>
                                    <x-ui.status-badge :label="$candidate['status']" tone="surface" />
                                </div>
            ...
            <div class="event-dashboard__columns">
                <x-ui.content-panel section="event-reschedule" title="Reschedule">
                    <form method="POST" action="{{ $organizerTools['reschedule_action'] }}" class="event-message-form section-body">
                        ...
                        <x-ui.action-control type="submit" label="Reschedule and notify" icon="calendar-sync" variant="primary" />
                    </form>
                </x-ui.content-panel>

                <x-ui.content-panel section="event-history" title="Change history">
    ...
    @else
        <div class="event-dashboard__overview">
            <div class="event-dashboard__main">
                <x-ui.content-panel section="event-about" eyebrow="{{ \Illuminate\Support\Str::headline($event['event_type']) }}" title="What to expect">
                    <p class="event-dashboard__copy">{{ $event['long_description'] }}</p>
                    <x-ui.icon-list :items="$content['safety']" class="section-body" />
                </x-ui.content-panel>

                <x-ui.content-panel section="event-schedule-preview" eyebrow="Program" title="The day at a glance">
            ...
            <aside class="event-dashboard__aside">
                ...
                <x-ui.content-panel section="event-organizers" title="Organizers">
        ...
        <x-ui.content-panel section="event-faq" eyebrow="Before you go" title="Frequently asked questions">

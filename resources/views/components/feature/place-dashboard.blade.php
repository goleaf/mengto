...
<div class="place-dashboard">
    @if ($activeTab === 'overview')
                ...
                <aside class="place-visit-panel" aria-labelledby="place-visit-actions-title">
                    ...
                    @else
                        <form method="POST" action="{{ route('actions.perform') }}" class="place-check-in-form">
                            ...
                            <x-ui.action-control type="submit" label="Check in for 2 hours" icon="map-pin-check" variant="primary" size="compact" />
                        </form>
                    @endif
        ...
        <section class="place-section" aria-labelledby="place-social-title">
            <header class="place-section__heading">
                ...
                <x-ui.action-control :href="$eventUrl" label="Create event here" icon="calendar-plus" variant="surface" size="compact" />
            </header>
            ...
            <div class="place-social-grid">
                <div class="place-social-grid__friends">
                    @forelse ($content['social']['friends'] as $friend)
                        <div class="place-friend">
                            <x-ui.initials-avatar :initials="$friend['initials']" size="sm" />
                            <div>
                ...
                <form method="POST" action="{{ route('actions.perform') }}" class="place-invite-form">
                    ...
                    <x-ui.action-control type="submit" label="Send privately" icon="send" variant="primary" size="compact" />
                </form>
            </div>
        </section>
        ...
        <section class="place-section" aria-labelledby="place-weather-title">
            <header class="place-section__heading">
                ...
                <x-ui.status-badge :label="$content['weather']['source']" icon="cloud-sun" tone="neutral" />
            </header>
    ...
    @elseif ($activeTab === 'photos')
        <section class="place-section" aria-labelledby="place-photos-title">
            ...
            <div class="place-gallery">
                ...
                @empty
                    <x-ui.empty-state icon="image-off" title="No photos yet" description="Recent, dated place photos will appear here." />
                @endforelse
            </div>
        </section>
            ...
            <div class="place-service-list">
                @forelse ($content['services'] as $service)
                    <article>
                        ...
                        <x-ui.status-badge :label="$service['status']" tone="neutral" />
                    </article>
                @empty
                    <x-ui.empty-state icon="package-open" title="No services listed" description="Contact the place before making a special trip." />
                @endforelse
            </div>
        </section>
    @elseif ($activeTab === 'rules')
        <section class="place-section" aria-labelledby="place-rules-title">
            <header class="place-section__heading">
                ...
                <x-ui.action-control :href="$correctionUrl" label="Correct a rule" icon="file-check-2" variant="surface" size="compact" />
            </header>
    ...
    @elseif ($activeTab === 'hours')
        <section class="place-section" aria-labelledby="place-hours-title">
            <header class="place-section__heading">
                ...
                <x-ui.action-control :href="$correctionUrl" label="Correct hours" icon="clock-arrow-up" variant="surface" size="compact" />
            </header>
        ...
        <section class="place-section" aria-labelledby="place-specialists-title">
            ...
            <div class="place-specialist-list">
                @forelse ($content['specialists'] as $specialist)
                    <article>
                        <x-ui.initials-avatar :initials="$specialist['initials']" size="md" />
                        <div>
                        ...
                        <x-ui.status-badge :label="$specialist['verification']" icon="badge-check" tone="neutral" />
                    </article>
                @empty
                    <x-ui.empty-state icon="user-round-search" title="No specialists listed" description="Call the place to confirm who is available." />
                @endforelse
            </div>
        </section>
    @elseif ($activeTab === 'reviews')
        <section class="place-section" aria-labelledby="place-reviews-title">
            <header class="place-section__heading">
                ...
                <x-ui.action-control :href="$reviewUrl" label="Write review" icon="star" variant="primary" size="compact" />
            </header>
            <div class="place-review-list">
                @forelse ($content['reviews'] as $review)
                    <article>
                        <header>
                            <x-ui.initials-avatar :initials="$review['initials']" size="sm" />
                            <div>
                ...
                @empty
                    <x-ui.empty-state icon="message-square-off" title="No reviews yet" description="New places remain discoverable without a rating." />
                @endforelse
            </div>
        </section>
        ...
        <section class="place-section" aria-labelledby="place-events-title">
            <header class="place-section__heading">
                ...
                @if ($place['allow_events'])
                    <x-ui.action-control :href="$eventUrl" label="Create event here" icon="calendar-plus" variant="primary" size="compact" />
                @endif
            </header>
    ...
    @elseif ($activeTab === 'questions')
        <section class="place-section" aria-labelledby="place-questions-title">
            <header class="place-section__heading">
                ...
                <x-ui.action-control :href="$questionUrl" label="Ask question" icon="message-circle-question" variant="primary" size="compact" />
            </header>
                    ...
                    <article>
                        ...
                        @elseif ($canManage)
                            <form method="POST" action="{{ route('actions.perform') }}">
                                ...
                                <x-ui.action-control type="submit" label="Publish answer" icon="send" variant="primary" size="compact" />
                            </form>
                        @else
                            <x-ui.status-badge label="Awaiting an answer" icon="clock-3" tone="neutral" />
                        @endif
                    </article>
                @empty
                    <x-ui.empty-state icon="circle-help" title="No questions yet" description="Ask about access, rules, hours, or facilities." />
                @endforelse
            </div>
        </section>
    ...
    @elseif ($activeTab === 'updates')
        <section class="place-section" aria-labelledby="place-updates-title">
            <header class="place-section__heading">
                ...
                <x-ui.action-control :href="$warningUrl" label="Report hazard" icon="triangle-alert" variant="danger" size="compact" />
            </header>
            ...
            <div class="place-warning-list">
                ...
                @empty
                    <x-ui.empty-state icon="shield-check" title="No active warnings" description="Current place conditions still need normal personal judgment." />
                @endforelse
            </div>
            ...
            @if (! $place['owner_managed'])
                <div class="place-claim">
                    ...
                    <x-ui.action-control :href="$claimUrl" label="Claim profile" icon="badge-check" variant="surface" size="compact" />
                </div>
            @endif
    ...
    @elseif ($activeTab === 'corrections')
        <section class="place-section" aria-labelledby="place-corrections-title">
            <header class="place-section__heading">
                ...
                <x-ui.action-control :href="$correctionUrl" label="Suggest correction" icon="file-check-2" variant="primary" size="compact" />
            </header>
                ...
                @forelse ($corrections as $correction)
                    <article>
                        <header>
                            ...
                            <x-ui.status-badge :label="\Illuminate\Support\Str::headline($correction['status'])" tone="neutral" />
                        </header>
                        <p>{{ $correction['proposed_value'] }}</p>
                        <small>Evidence: {{ $correction['evidence'] }} · {{ $correction['created_at'] }}</small>
                    </article>
                @empty
                    <x-ui.empty-state icon="file-check-2" title="No pending corrections" description="Important changes require evidence and review." />
                @endforelse
            </div>
        </section>
    @endif
    ...
    <footer class="place-dashboard__safety">
        ...
        <div>
            <x-ui.action-control :href="$correctionUrl" label="Correct data" icon="file-check-2" variant="ghost" size="compact" />
            <x-ui.action-control :href="$reportUrl" label="Report place" icon="flag" variant="ghost" size="compact" />
        </div>
    </footer>
</div>

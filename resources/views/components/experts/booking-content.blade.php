    ...
    <div class="grid content-start gap-7">
        ...
        <section aria-labelledby="appointment-heading">
            <div class="flex flex-wrap items-start justify-between gap-3">
                ...
                <x-ui.status-badge :label="$booking['status']" icon="calendar-clock" :tone="$booking['status_value'] === 'completed' ? 'success' : 'surface'" />
            </div>
        ...
        @if ($consultation)
            <section class="border-y border-paw-line py-6" aria-labelledby="summary-heading">
                <div class="flex flex-wrap items-center justify-between gap-3">
                    ...
                    <x-ui.status-badge :label="$consultation['status']" icon="clipboard-check" />
                </div>
                ...
                @forelse ($documents as $document)
                    <article class="text-sm">
                        <div class="flex items-start justify-between gap-3">
                            ...
                            <x-ui.status-badge :label="$document['revoked'] ? 'Revoked' : 'Active'" :icon="$document['revoked'] ? 'lock' : 'file-lock-2'" />
                        </div>

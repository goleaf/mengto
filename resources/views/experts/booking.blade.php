    ...
    <div class="grid gap-6">
        <header class="flex flex-wrap items-center justify-between gap-3 border-b border-paw-line pb-5">
            ...
            @if ($consultation && $booking['format'] === 'Video')
                <x-ui.action-control label="Open consultation room" icon="video" variant="primary" :href="route('consultations.show', $consultation['id'])" />
            @endif
        </header>

        <x-experts.partials.booking-content
            :booking="$booking"
            :expert="$expert"
            :service="$service"
